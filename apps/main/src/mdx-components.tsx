...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    a: ({ children, href }) => <a href={href} className="text-neutral-600 dark:text-neutral-300 underline hover:text-emerald-600 dark:hover:text-emerald-300">{children}</a>,
    p: ({ children }) => <p className="my-1.5">{children}</p>,
    blockquote: ({ children }) => <blockquote className="my-6 pl-4 border-l-4 border-emerald-300 dark:border-emerald-700">{children}</blockquote>,
